...
def averages(data, n=None):
    ...
    if n:
        if n < 1:
            print("Number of data to consider can not be less than one!")
            assert False
        if n > l:
            n = l
        data = data[-n:]
...
def display_averages(averages):
    ...
    """
    return "{:.0f}/{:.0f} {:.0f}".format(*averages)
...
def set_data_file(args):
    ...
    if args.file == data_file:
        print("Using '{}' as data file.".format(data_file))
    else:
        print("Reassigned data file to '{}'.".format(args.file))
...
def add(args):
    ...
    print("Recording BP of {}/{}.".format(sp, dp))
    print("{} / {}".format(get_category(sp, "s"), get_category(dp, "d")))
